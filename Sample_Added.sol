...
@param token The address of an ERC-1155 collection contract containing the
  ...
  */
    ...
    // Function initialising variables used throughout contract.
  ...
  */
    function purchaseItem(uint256 _itemId, uint256 _amount, uint256 _assetId) external payable {
        ...
        if ((sellingPair.assetType == 1) == false) {
            ...
            balance = sellingAsset.balanceOf(msg.sender);
        }

        uint256 newPrice = sellingPair.price.mul(_amount);

        require(balance >= newPrice,
        "Not enough token/ether to complete purchase.");

        uint256 feeValue = newPrice.mul(feePercent).div(100000);
        uint256 royaltyValue = newPrice.mul(itemRoyaltyPercent).div(100000);
